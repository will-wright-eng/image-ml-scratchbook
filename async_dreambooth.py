...
class replicateApi:
    def __init__(self):
        self.model_owner = os.environ.get("MODEL_OWNER")
        self.model_name = os.environ.get("MODEL_NAME")
        self.model_slug = f"{self.model_owner}/{self.model_name}"
        self.img_urls = []
        self.results = {}
    ...
    async def generate_dreambooth_img(self, prompt):
        ...
        self.results[str(uuid.uuid4())] = {
            "prompt": prompt,
            "img_url_output": image_urls,
        }

    def img_urls_from_results(self):
        self.img_urls = [
            resultdict.get("img_url_output", None)
            for resultid, resultdict in self.results.items()
        ]
    ...
    def get_model_versions(self):
        url = f"https://api.replicate.com/v1/models/{self.model_slug}/versions"
    ...
    def download_all_predictions(self):
        NoneType = type(None)
        nested_list = [i.get("output") for i in x]
        nested_list = [i for i in y if not isinstance(i, NoneType)]
